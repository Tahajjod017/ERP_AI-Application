...
namespace GCTL.Service.MasterSetup.EmployeeTypes
{
    public class EmployeeTypesService : AppService<EmployeeType>, IEmployeeTypesService
    {
        ...
        #region Soft Delete
        public async Task<EmployeeTypesVM> SoftDeleteAsync(BaseViewModel model, List<int> ids)
        {
            await _genericRepository.BeginTransactionAsync();
            try
            {
                var data = await _genericRepository.FindAsync(x => ids.Contains(x.EmployeeTypeID));
                ...
                await _userInfoService.ActionLogDeleteAsync("Employee Type", ActionName.DataDeleted, null, beforeEntity, targetIds, model);
