...
namespace GCTL.Service.MasterSetup.EmploymentNatures
{
    public class EmploymentNatureService : AppService<EmploymentNature>, IEmploymentNatureService
    {
        ...
        {
            var existingName = await _genericRepository.FindAsync(b => b.EmploymentNatureName == name);
        ...
        #region Soft Delete
        public async Task<EmploymentNatureVM> SoftDeleteAsync(BaseViewModel model, List<int> ids)
        {
            await _genericRepository.BeginTransactionAsync();
            try
            {
                var data = await _genericRepository.FindAsync(x => ids.Contains(x.EmploymentNatureID));
                ...
                await _userInfoService.ActionLogDeleteAsync("Employment Nature", ActionName.DataDeleted, null, beforeEntity, targetIds, model);
