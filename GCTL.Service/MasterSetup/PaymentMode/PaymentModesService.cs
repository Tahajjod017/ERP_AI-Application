...
namespace GCTL.Service.MasterSetup.PaymentMode
{
    public class PaymentModesService : AppService<PaymentModes>, IPaymentModeService
    {
        ...
        {
            var existingNames = await _genericRepository.FindAsync(b => b.DeletedAt == null && b.PaymentModeName != null);

            var nameList = existingNames.Select(b => b.PaymentModeName);
        ...
        #region Soft Delete
        public async Task<PaymentModeVM> SoftDeleteAsync(DeleteRequestVM requestVM)
        {
            await _genericRepository.BeginTransactionAsync();
            try
            {
                var data = await _genericRepository.FindAsync(x => requestVM.Ids.Contains(x.PaymentModeID));
                ...
                await _userInfoService.ActionLogDeleteAsync("Payment Mode", ActionName.DataDeleted, null, beforeEntity, targetIds, requestVM);
