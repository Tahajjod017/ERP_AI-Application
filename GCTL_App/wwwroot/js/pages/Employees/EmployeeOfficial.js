$(document).ready(function () {
    ...
    //        const choicesInstance = choicesInstances[fieldId];
    //        if (choicesInstance) {
    //            choicesInstance.clearStore();
    //            choicesInstance.setChoices([{
    //                value: '',
    //                label: selectedId && empID ? 'Loading...' : 'Please select Organization and Employee first',
    //                disabled: true
    //            }], 'value', 'label', true);
    //            if (selectedId && empID) {
    //                // Reset pagination state
    //                choicesInstance.currentPage = 1; // Add this to reset page
    //                choicesInstance.lastSearch = ''; // Reset search term
    //                choiceManager.populator.populateFromApi(fieldId, `/EmployeeOfficial/GetEmployeeSupDDbyComp?id=${encodeURIComponent(selectedId)}&empID=${encodeURIComponent(empID)}&page=1&pageSize=50`, {
    //                    labelKey: 'label',
    //                    valueKey: 'value',
    //                    placeholder: choiceManager.getPlaceholderFromHtml(fieldId) || 'Select Supervisor...'
    //                }).then(success => {
    //                    if (!success) {
    //                        choicesInstance.clearStore();
    //                        choicesInstance.setChoices([{
    ...
    //    const selectedId = $(this).val();
    //    const choicesInstance = choicesInstances['HeadOfDepartmentId'];
    //    if (choicesInstance) {
    //        choicesInstance.clearStore();
    //        choicesInstance.setChoices([{
    //            value: '',
    //            label: selectedId ? 'Loading...' : 'Please select Department first',
    //            disabled: true
    //        }], 'value', 'label', true);
    //        if (selectedId) {
    //            // Reset pagination state
    //            choicesInstance.currentPage = 1; // Add this to reset page
    //            choicesInstance.lastSearch = ''; // Reset search term
    //            choiceManager.populator.populateFromApi('HeadOfDepartmentId', `/EmployeeOfficial/GetEmployeeHOD?id=${encodeURIComponent(selectedId)}&page=1&pageSize=50`, {
    //                labelKey: 'label',
    //                valueKey: 'value',
    //                placeholder: choiceManager.getPlaceholderFromHtml('HeadOfDepartmentId') || 'Select Head of Department...'
    //            }).then(success => {
    //                if (!success) {
    //                    choicesInstance.clearStore();
    //                    choicesInstance.setChoices([{
    ...
    //    ['SeniorSupervisorId', 'ImmediateSupervisorId'].forEach(fieldId => {
    //        const choicesInstance = choicesInstances[fieldId];
    //        if (choicesInstance && selectedId && orgId) {
    //            choicesInstance.clearStore();
    //            choicesInstance.setChoices([{
    //                value: '',
    //                label: 'Loading...',
    //                disabled: true
    //            }], 'value', 'label', true);
    //            choiceManager.populator.populateFromApi(fieldId, `/EmployeeOfficial/GetEmployeeSupDDbyComp?id=${encodeURIComponent(orgId)}&empID=${encodeURIComponent(selectedId)}&page=1&pageSize=50`, {
    //                labelKey: 'label',
    //                valueKey: 'value',
    //                placeholder: choiceManager.getPlaceholderFromHtml(fieldId) || 'Select Supervisor...'
    //            }).then(success => {
    ...
    //            choicesInstance.setChoices([{
    //                value: '',
    //                label: 'Please select Organization and Employee first',
    //                disabled: true
    //            }], 'value', 'label', true);
    //        }
    //    });
    ...
    $("#OrganizationID").change(function () {
        ...
        ['SeniorSupervisorId', 'ImmediateSupervisorId'].forEach(fieldId => {
            ...
            if (paginatedInstance && paginatedInstance.refresh) {
                // ✅ Use the refresh method to reload with pagination intact
    ...
    // Load initial employee if ID is in URL
    const lastInt = getLastIntFromUrl();
    if (lastInt) {
        loadEmployeeById(lastInt).then(() => {
            if (choicesInstances['EmployeePersonalId']) {
                console.debug(`Setting EmployeePersonalId to ${lastInt}`);
                choiceManager.setChoiceValue('EmployeePersonalId', lastInt.toString());
            }
    ...
    function populateChoicesDropdowns(response) {
        ...
        dropdownMappings.forEach(mapping => {
            if (mapping.value) {
                choiceManager.setChoiceValue(mapping.id, mapping.value.toString());
            }
        });
    }
...
//        // Load the employee into the dropdown first
//        loadEmployeeById(lastInt).then(() => {
//            debugger
//            if (employeeChoices) {
...
//    function initEmployeeChoices() {
//        const selectEl = document.getElementById('EmployeePersonalId');

//        if (!selectEl) {
//            console.error('EmployeePersonalId select element not found');
//            return;
//        }

//        employeeChoices = new Choices(selectEl, {
//            searchEnabled: true,
//            placeholder: true,
//            placeholderValue: 'Select Employee...',
...
//            loading = true;
//            try {
//                const res = await fetch(`/EmployeePersonal/SearchEmployeeDD?search=${encodeURIComponent(search)}&page=${page}&pageSize=${pageSize}`);
...
//                // If search is completely cleared, reload initial employees without debounce
//                if (searchTerm.length === 0 && initialLoadDone) {
//                    currentPage = 1;
//                    lastSearch = '';

//                    fetchOptions('', 1, 50).then(data => {
//                        if (data.items && data.items.length > 0) {
//                            employeeChoices.setChoices(data.items, 'value', 'label', true);
//                        }
//                    });
//                    return;
//                }
...
//                    employeeChoices.setChoices(data.items, 'value', 'label', false);
//                }
//            }
//        }
...
//            // Load initial 50 employees on first dropdown open
//            if (!initialLoadDone) {
//                currentPage = 1;
//                lastSearch = '';

//                const data = await fetchOptions('', 1, 50);
