$(document).ready(function () {
            ...
            success: function (res) {
                ...
                if (res && res.length > 0) {
                    ...
                    res.forEach(function (item, i) {
                        ...
                        // FIXED: Changed empBenefitVMM to empAllowanceVMM
                        ...
                        // FIXED: Changed field names to match ViewModel
                                    ...
                                    <div class="row g-3 houseRentRow">
                                        <div class="col-lg-4 col-md-6 col-sm-12">
                                            ...
                                            <input type="number" class="form-control" 
                                                   name="Allowances[${i}].EmployeeAllowanceSetups[0].SalaryMin" 
                                                   placeholder="Enter Min Salary" />
                                        </div>
                                        <div class="col-lg-4 col-md-6 col-sm-12">
                                            <label class="form-label">Max Salary</label>
                                            <input type="number" class="form-control" 
                                                   name="Allowances[${i}].EmployeeAllowanceSetups[0].SalaryMax" 
                                                   placeholder="Enter Max Salary" />
                                        </div>
                                        <div class="col-lg-3 col-md-3 col-sm-12 d-flex align-items-center gap-3">
                                            <div class="input-group mb-3">
                                                <input type="hidden" 
                                                       name="Allowances[${i}].EmployeeAllowanceSetups[0].CalculationTypeID" 
                                                       value="" 
                                                       class="calculationTypeHidden" />
                                                ...
                                                <div class="percentRate" style="display: none;">
                                                    <label class="form-label">Percentage(%)</label>
                                                    <select class="form-select choiceDD percentInput" 
                                                            name="Allowances[${i}].EmployeeAllowanceSetups[0].Value" 
                                                            style="width:160px;">
                                                        <option value="">Select %</option>
    ...
    //
    // Numeric validation for Fixed and Percentage inputs
    $(document).on('input', '.fixedInput, .percentInput', function () {
        let oldValue = this.value;
        let newValue = oldValue.replace(/[^0-9.]/g, ''); // allow numbers and decimal
        if (oldValue !== newValue) {
            toastr.warning("Only numbers are allowed!");
            this.value = newValue;
        }
    ...
    // Save allowance for a single accordion card
    $(document).on('click', '.PayRollEmpAllowanceSave', function (e) {
        ...
        // Construct allowance object
        let allowance = {
            EmployeeAllowanceID: parseInt($card.find('input[name$=".EmployeeAllowanceID"]').val()) || 0,
            EmployeeAllowanceTypeID: parseInt($card.find('input[name$=".EmployeeAllowanceTypeID"]').val()) || 0,
        ...
        $card.find('.houseRentContainer .houseRentRow').each(function () {
            ...
            allowance.AllowanceSetups.push({
                EmployeeAllowanceSetupID: parseInt($row.find('input[name$=".EmployeeAllowanceSetupID"]').val()) || 0,
                SalaryMin: salaryMin,
                SalaryMax: salaryMax,
        ...
        jsonData.Allowances.push(allowance);
        console.log("Sending JSON for this card:", JSON.stringify(jsonData, null, 2));
        // AJAX call
        $.ajax({
