...
function initCreateJobModal() {
    ...
    $('#DivisionId').select2({
        ...
        width: '100%',
        dropdownParent: $('#DivisionId').closest('.modal').length ? $('#DivisionId').closest('.modal') : null,
        ajax: {
            url: '/CreateJobs/GetDivisions',
            dataType: 'json',
            delay: 250,
            data: function (params) {
                return {
    ...
    $('#JobTypeID').select2({
        ...
        width: '100%',
        dropdownParent: $('#JobTypeID').closest('.modal').length ? $('#JobTypeID').closest('.modal') : null,
        ajax: {
            url: '/CreateJobs/GetJobTypes',
            dataType: 'json',
            delay: 250,
            data: function (params) {
                return {
    ...
    $('#StatusID').select2({
        ...
        width: '100%',
        dropdownParent: $('#StatusID').closest('.modal').length ? $('#StatusID').closest('.modal') : null,
        ajax: {
            url: '/CreateJobs/GetStatuses',
            dataType: 'json',
            delay: 250,
            data: function (params) {
                return {
    ...
    $(ids.submitBtn).on("click", function (e) {
        e.preventDefault();
        try {
            const createJobFrom = document.getElementById("CreateJob-form")
            const formData = new FormData(createJobFrom);
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.success) {
                        ...
                        resetForm();
                        debugger;
                        if (typeof finishModalProcess === "function") {
                            finishModalProcess(value, text);
                        } 
                    } else {
    ...
    window.LoadMainPageData = function (customerId) {
        $.ajax({
...
window.initCreateJobModal = initCreateJobModal;