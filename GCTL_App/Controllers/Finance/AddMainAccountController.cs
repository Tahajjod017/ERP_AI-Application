...
namespace GCTL_App.Controllers.Finance
{
    public class AddMainAccountController : BaseController
    {
        ...
        #region GetAll
        public async Task<IActionResult> GetAll(int pageNumber = 1, int pageSize = 5, string searchTerm = "", string sortColumn = "MainAccountID", string sortOrder = "desc")
        {
            var result = await _addMainAccountService.GetAllAsync(pageNumber, pageSize, searchTerm, sortColumn, sortOrder);
        ...
        public async Task<IActionResult> GetAccountGroupByClassId(int id)
        {
            var result = await _commonService.GetAccountGroupByClassId(id);
            return Json(result);
        }
        #endregion
    }
