...
{
    public class ActionLogExceptionController : BaseController
    {
        private readonly IActionLogService actionLogService;
        private readonly AppDbContext appDbContext;

        public ActionLogExceptionController(ITranslateService translateService, IUserProfileService userProfileService, IActionLogService actionLogService, AppDbContext appDbContext) : base(translateService, userProfileService)
        {
        ...
        public async Task<IActionResult> Index()
        {
            try
            {
                var targetTypes = await appDbContext.ActionLogs
                    .Where(a => !string.IsNullOrEmpty(a.TargetType) && a.ActionName== "Error Exception")
                    .Select(a => a.TargetType).Distinct().ToListAsync();
                
                var actionNames = await appDbContext.ActionLogs .Where(x => x.ActionName == "Error Exception").Select(a => a.ActionName).Distinct() .ToListAsync();


                var userNameEmail = await appDbContext.ActionLogs
                    .Include(x => x.CreatedByNavigation)
                    .Where(x => x.CreatedByNavigation != null && x.ActionName== "Error Exception").GroupBy(x => x.CreatedBy)
                    .Select(g => new
                    {
                        CreatedBy = g.Key,
                ...
                ViewBag.ActionNameDD = new SelectList(actionNames.Select(x => new { Value = x, Text = x }), "Value", "Text", actionNames.FirstOrDefault());
        ...
        public async Task<IActionResult> ActionLogDataTable(int pageNumber = 1, int pageSize = 5, string searchTerm = "", string currentSortColumn = "", string currentSortOrder = "", DateTime? fromDate = null, DateTime? toDate = null, string? tergetType = null, string? actionName = null, int? createdBy = null)
        {
            ...
            var result = await actionLogService.GetPaginateActionLogException(pageNumber, pageSize, searchTerm, currentSortColumn, currentSortOrder, fromDate, toDate, tergetType, actionName, createdBy);
            return Json(result);
